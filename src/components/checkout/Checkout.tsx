import { Flex, FormControl, Input, Button, Select, Box } from '@chakra-ui/react';
import { useForm } from 'react-hook-form';
import { emailPattern, states } from '../../config/staticData';
import * as Yup from 'yup'
import { AddressForm } from '@form-types';
import { yupResolver } from '@hookform/resolvers/yup'

export const Checkout: React.FC = (): JSX.Element => {

    const validationSchema = Yup.object().shape({
        email: Yup.string().matches(emailPattern, {message: 'Enter a valid email'}).required('Field is required'),
        ...
        zip: Yup.string().test('len', 'Must be 5 characters long', val => val?.length === 5).required('Field is required'),
        phone: Yup.string().test('len', 'Must be 10 characters long', val => val?.length === 10).required('Field is required'),
    })
	...
	return (
		<Flex
            ...
            direction='column'
            gap='50px'>
            <Box border='1px solid red' padding='5px' borderRadius='3px'>Shipping/Billing Address Details</Box>
